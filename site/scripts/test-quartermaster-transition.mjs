...
try {
  ...
  await admin.query("alter table public.member add column display_name text default 'Existing member'");
  const beforeMigration = (await admin.query('select count(*)::int as rows, sum(balance)::int as total from public.economy_wallet')).rows[0];
  await admin.query(readFileSync(new URL('../db/0056_quartermaster_transition.sql', import.meta.url), 'utf8'));
  const afterMigration = (await admin.query('select count(*)::int as rows, sum(balance)::int as total from public.economy_wallet')).rows[0];
  assert.deepEqual(afterMigration, beforeMigration); pass('transition preserves all existing wallet rows and balances');
  await expectDatabaseError(admin.query('update public.economy_wallet set balance=balance+10'), /moved to the full economy/);
  pass('old wallet writes cannot spend or award balances after transition');
  const member = (await admin.query('select discord_id from public.member where id=$1', [ids.claim])).rows[0];
  const migrated = (await admin.query('select public.quartermaster_legacy_account($1) as value', [member.discord_id])).rows[0].value;
  assert.equal(migrated.balance, (await admin.query('select balance from public.economy_wallet where member_id=$1', [ids.claim])).rows[0].balance);
  assert.ok(migrated.lastDailyAt); pass('server export retains opening balance and daily claim time');
  const grants = (await admin.query("select has_function_privilege('anon','public.quartermaster_legacy_account(text)','execute') as anon, has_function_privilege('authenticated','public.quartermaster_legacy_account(text)','execute') as member, has_function_privilege('service_role','public.quartermaster_legacy_account(text)','execute') as service")).rows[0];
  assert.deepEqual(grants, {anon:false, member:false, service:true}); pass('legacy export is available only to the trusted server role');
  await admin.end();
