type ForageAvailability = {
  frozen: boolean; busy: boolean; unknown: boolean; billet: boolean; forageWait: string;
  hasPeers: boolean;
...
export function forageBlockers(state: ForageAvailability): string[] {
  ...
  if (state.billet) reasons.push(`Your Billet protection is on, so you cannot forage. Turn it off below to remove protection immediately.`);
