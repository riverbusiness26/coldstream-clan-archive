...
export type QuartermasterSnapshot = { profile: Profile; cooldowns: Partial<Record<'work' | 'daily' | 'steal' | 'crime' | 'withdraw' | 'anchor' | 'vingt' | 'heist', string | null>>; catalogue: CatalogueItem[]; inventory: KitItem[]; ledger: LedgerEntry[]; forageAlerts?: ForageAlert[]; leaderboard: BoardMember[]; game: VingtGame | null; social: SocialSnapshot; config: QuartermasterConfig; serverTime: string; preview: boolean };
...
export function actPreview(current: QuartermasterSnapshot, action: ActionKey, args: ActionArgs): QuartermasterResult {
  ...
  else if (action === 'anchor') { ready('anchor'); const value = stake('anchor'); if (!seed.anchor_symbols.includes(String(args.symbol))) reject('Choose a symbol on the cloth.'); const rolls = Array.from({ length: 3 }, () => seed.anchor_symbols[Math.floor(Math.random() * seed.anchor_symbols.length)]); const matches = rolls.filter((symbol) => symbol === args.symbol).length; const payout = matches ? value * c.anchor.payouts[String(matches) as '1' | '2' | '3'] : 0; p.purse += payout - value; cooldown('anchor', c.anchor.command_cooldown_seconds); resultGame = { rolls, matches, payout, stake: value }; message = `You rolled ${rolls.join(' · ')}. ${matches ? `${shillings(payout)} returned, including your stake.` : 'The dealer takes the stake.'}`; }
  else if (action === 'vingt') {
    if (!args.move) {
      if (isVingtActive(snapshot.game)) reject('Finish your current hand first.'); ready('vingt'); const value = stake('vingt'); p.purse -= value;
      ...
      if (snapshot.game.playerTotal === 21) { p.purse += Math.floor(value * c.vingt.natural_multiplier); snapshot.game.status = 'natural'; snapshot.game.payout = Math.floor(value * c.vingt.natural_multiplier); snapshot.game.net = snapshot.game.payout - value; message = 'Twenty-one! Your winnings have been added to your Wallet.'; cooldown('vingt', c.vingt.command_cooldown_seconds); }
    } else {
      ...
      if (game!.playerTotal > 21 || args.move === 'stand' || args.move === 'double' || game!.playerTotal === 21) {
        ...
        game!.status = won ? 'won' : push ? 'push' : 'lost'; game!.payout = won ? game!.stake * c.vingt.win_multiplier : push ? game!.stake : 0; game!.net = game!.payout - game!.stake; p.purse += game!.payout; cooldown('vingt', c.vingt.command_cooldown_seconds); message = won ? 'You won. Your winnings have been added to your Wallet.' : push ? 'An even hand. Your stake is returned.' : 'You lost this hand. Your bet has been spent.';
