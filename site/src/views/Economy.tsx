...
const knownFailure: Record<string, string> = {
  ...
  ECONOMY_REQUEST_KEY_CONFLICT: 'That request could not be safely repeated. Please refresh the stores.',
  ECONOMY_REQUEST_KEY_INVALID: 'That request was rejected. Please refresh the stores and try again.',
};
...
export default function Economy({ demo = DEMO }: { demo?: boolean }) {
  ...
  const load = useCallback(async () => { if (demo || !supa) return; setLoading(true); setError(null); try { setSnapshot(await readEconomySelf(supa)); } catch (e) { setError(e instanceof Error ? e.message : 'The stores are unavailable.'); } finally { setLoading(false); } }, [demo]);
  ...
  const runAction = async (kind: 'daily' | 'purchase' | 'equip', slug?: string, slot?: EconomySlot) => {
    ...
    try {
      ...
      if (!supa) throw new Error('The stores are unavailable.');
  ...
  if (loading) return <main className="economy-page wrap solo" aria-busy="true"><p className="cg-eyebrow">Quartermaster's Stores</p><h1>Opening the stores.</h1><p role="status">Loading your balance and collection…</p></main>;
  if (error && !snapshot) return <main className="economy-page wrap solo"><p className="cg-eyebrow">Quartermaster's Stores</p><h1>The stores are unavailable.</h1><p role="alert">{error}</p><button className="hq-button" onClick={() => void load()}>Refresh stores</button></main>;
  if (!snapshot) return null;
  return <main className="economy-page wrap solo"><header className="economy-hero"><div><p className="cg-eyebrow">Member economy</p><h1>{snapshot.shop_display_name}</h1><p className="page-sub">Collect small marks of service for your member display.</p></div><div className="economy-balance"><span>Your balance</span><strong>{snapshot.balance} <small>{snapshot.currency_display_name}</small></strong><button className="hq-button primary" disabled={action?.status === 'pending'} onClick={() => void runAction('daily')}>Claim daily issue</button></div></header>
