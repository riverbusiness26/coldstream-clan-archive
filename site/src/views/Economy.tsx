...
export default function Economy({ demo = DEMO }: { demo?: boolean }) {
  ...
  return <main className="economy-page wrap solo"><header className="economy-hero"><div><p className="cg-eyebrow">Member economy</p><h1>Shillings</h1><p className="page-sub">Your balance, collection and access to the Quartermaster's Stores.</p></div><div className="economy-balance"><span>Your balance</span><strong>{snapshot.balance} <small>{snapshot.currency_display_name}</small></strong><button className="hq-button primary" disabled={action?.status === 'pending'} onClick={() => void runAction('daily')}>Claim daily issue</button></div></header>
