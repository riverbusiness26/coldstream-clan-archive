...
import { weeklyMediaItems, nextMediaIndex, featuredMemberName, type WeeklyMedia, type WeeklyFeature } from '../lib/weeklyMedia';
...
export function HomeFilm({ controls = true, weekly, mode = 'normal', autoPlay = false, onFeaturedMemberChange }: { onFeaturedMemberChange?: (name: string | null) => void; controls?: boolean; weekly?: WeeklyFeature[]; mode?: 'normal' | 'expanding'; autoPlay?: boolean } = {}) {
  ...
  const featuredName = featuredMemberName(media);
  useEffect(() => { onFeaturedMemberChange?.(featuredName); }, [featuredName, onFeaturedMemberChange]);
...
export default function Home({ me, signIn, signOut, embedded = false }: { me: Me | null; go: (v: string) => void; signIn: () => void; signOut: () => void; embedded?: boolean }) {
  ...
  const [featuredMember, setFeaturedMember] = useState<string | null>(null);
