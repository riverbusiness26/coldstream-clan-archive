...
export default function Admin({ me, signOut }: { me: Me | null; signOut: () => void }) {
  ...
  const [statSubmitterFilter, setStatSubmitterFilter] = useState('all');
  ...
  const statSubmitters = useMemo(() => [...new Set(statSubmissions.map((submission) => submission.submitter_id))].map((id) => ({ id, name: memberById.get(id)?.display_name || 'Discord member' })).sort((a, b) => a.name.localeCompare(b.name)), [memberById, statSubmissions]);
  const selectableStats = statSubmitterFilter === 'all' ? statSubmissions : statSubmissions.filter((submission) => submission.submitter_id === statSubmitterFilter);
