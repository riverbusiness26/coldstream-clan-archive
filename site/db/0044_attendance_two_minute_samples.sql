-- The Discord bot records voice presence every two minutes. Keep member-level
-- totals derived from those samples only, with no manually entered hours.
...
begin
  ...
  return coalesce((
    select round(count(*)::numeric * 2 / 60, 1)
