...
test('the Shillings page is reachable only through the unlisted stores route', () => { assert.match(app, /view === 'stores'/); assert.match(routing, /'stores'/); assert.doesNotMatch(shell, /\['Stores', 'stores'\]/); assert.doesNotMatch(shell, /href="#\/stores"/); });
